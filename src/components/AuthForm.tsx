 ...
 *
 * Sending mail to somebody else's inbox reliably needs an authenticated domain,
 * and until that exists a confirmation step fails silently for every user who
 * is not the owner — which is worse than not asking. So signup completes
 * immediately.
 *
 * The password rules stay, because that is what actually guards an account:
 * confirmation only proves an address belongs to the person, it does nothing
 * to protect the account afterwards. If confirmation is switched back on in
 * Supabase, signUp returns no session and the notice below covers it.
 */
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        ...
        // Only reachable if confirmation is switched on in Supabase.
        if (!data.session) {
          setMode("signin");
